...
\small ChatGPT app collected for all 60 prompts (8 June, screenshot-backed). Validation gate passes: 60/60 rows, screenshots archived, transcription QA applied. The app model label is not reliably captured in-frame.
\end{block}
\end{column}
\begin{column}{0.32\textwidth}
...
\small Thesis draft compiles at \textbf{83 pages}: all chapters written, 4 figures, 25 tables, 81 metadata-verified references, all numbers script-produced from the pipeline.
\end{block}
\end{column}
\end{columns}
\vspace{0.2cm}
...
Thesis draft & outline & \textbf{83 pp., compiles clean, submission package ready} \\
\bottomrule
\end{tabular}
\end{frame}
...
ChatGPT app & model label not reliably visible & \textbf{60} & core \\
...
\begin{itemize}
  ...
  \item app model label treated as an unresolved deployment-stack limitation.
\end{itemize}
\end{column}
\end{columns}
...
\begin{column}{0.52\textwidth}
\begin{center}
\includegraphics[height=0.78\textheight]{../figures/within_between_scatter.png}
\end{center}
\end{column}
\end{columns}
...
%% ── SLIDE 9b: Session/prompt/interval robustness ────────────────────────
\begin{frame}[shrink=12]{Could it be the session, the prompts, or the interval method? Also no}
\framesubtitle{Three new robustness checks (10 June), all exploratory, all from existing data}
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.52\textwidth}
\small
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{p{0.42\columnwidth}llr}
\toprule
\textbf{Check} & \textbf{Subst.} & \textbf{Other} & $p$ \\
\midrule
...
\bottomrule
\end{tabular}\\[6pt]
\small Divergence does not cluster by collection position and is not predicted by shallow prompt-surface features --- it tracks \emph{what the prompts ask} (refusal pressure, factual support), consistent with the moderator pattern.
\end{column}
\begin{column}{0.44\textwidth}
\textbf{Interval robustness}\\[3pt]
\small Percentile bootstrap (20k resamples) reproduces Wilson: overall $[0.18,0.40]$ vs.\ $[0.185,0.408]$; tier-1 $[0.08,0.27]$ vs.\ $[0.09,0.28]$.\\[8pt]
\textbf{Four-channel style profile (descriptive)}\\[3pt]
\small Mean words per answer on the same 60 prompts: API 123 $\rightarrow$ Claude 156 $\rightarrow$ app 241 $\rightarrow$ Gemini 437. Response style is a channel trait on every surface --- one more reason verbosity is never counted as divergence.
\end{column}
\end{columns}
\slidefoot{Script: \texttt{analyze\_order\_features\_readability.py}; outputs under \texttt{analysis\_scaleup\_cleaned/order\_features\_readability/}. New Results subsection + table in the draft.}
\end{frame}
...
\begin{column}{0.52\textwidth}
\begin{center}
\includegraphics[height=0.74\textheight]{../figures/channel_agreement_heatmap.png}
\end{center}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}[shrink=12]{Reliability status: independent coding completed}
\framesubtitle{Strong core labels; safety framing remains weak}
\small
\renewcommand{\arraystretch}{1.25}
...
\textbf{Human inter-rater reliability} & 54 blind units coded independently. Refusal, format, and unsupported inference: $\kappa=1.00$; factuality: $\kappa=0.72$; UI surface: $\kappa=0.54$. & \oktext{Done} \\[2pt]
Soft-label warning & Safety-framing strength is weak ($\kappa=0.06$, weighted $\kappa=0.27$), so it stays descriptive and never carries a final claim alone. & \opentext{Caution} \\[2pt]
Unverified raters & Retained only as local diagnostics; not used as human IRR evidence. & \oktext{Separated} \\
\bottomrule
\end{tabularx}
\vspace{0.15cm}
\small \textbf{Implication:} the strongest claims should stay anchored in refusal status, factual support, format compliance, and visible UI evidence, not safety-framing strength alone.
\end{frame}
...
\small
\begin{itemize}
  \item \textbf{83 pages}, compiles clean: 0 undefined references, 4 figures, 25 tables.
...
\begin{itemize}
  ...
  \item \texttt{abstract\_en.txt} (375 words, per fact-sheet);
...
\begin{itemize}
  ...
  \item[\cmark] Human IRR completed: core labels strong/acceptable; UI surface moderate.
  \item[\cmark] Coding-dependence sensitivity: no final case rests on safety framing alone.
...
\begin{itemize}
  ...
  \item[\xmark] App-side stability unless the scoped 17-case ChatGPT repetition packet is collected.
  \item[\xmark] Strong standalone claims from safety-framing strength.
  \item[\xmark] Claude/Gemini \emph{app} claims; app sheets are prepared but uncollected and out of current scope.
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
2 & Reliability interpretation & Human IRR is now complete; question is whether the safety-framing caution is framed strongly enough. & Aleksandra \\[2pt]
3 & App-side repetitions & Re-run the 17 divergent prompts in the ChatGPT app, 2 reps. This is the only app collection worth prioritizing before submission; Claude/Gemini apps stay future work unless specifically requested. & me (decision: priority) \\[2pt]
...
\end{tabularx}
\vspace{0.15cm}
\small\textbf{Plan to 1 July:} this week: draft review + remaining app repetitions; next week: incorporate comments; final week: freeze, format check, submit from UZH address to \texttt{studies@ifi.uzh.ch}.
\end{frame}
