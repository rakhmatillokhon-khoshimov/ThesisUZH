...
PROMPT_BANK = ROOT / "pilot_prompt_bank.csv"
PILOT_OUTPUTS = ROOT / "pilot_outputs"
VALID_UI_SURFACES = {
    "none",
    "web_search",
    "map_widget",
...
def load_channel_rows(
    ...
    vibesdk_wrapper_log: Optional[Path] = None,
    ...
    candidates = {
        ...
        "sparkloom_wrapper": ["**/" + "vibe" + "sdk_wrapper_log_private.csv"],
    }
    explicit_paths = {
        ...
        "sparkloom_wrapper": vibesdk_wrapper_log,
    }
...
def make_api_only_rows(loaded: Dict[str, Dict[str, Dict[str, str]]]) -> List[Dict[str, str]]:
    rows: List[Dict[str, str]] = []
    for channel_key in ["openai_api", "claude_api", "gemini_api", "sparkloom_wrapper"]:
...
def main() -> int:
    ...
    parser.add_argument("--vibesdk-wrapper-log", type=Path, default=None)
    parser.add_argument(
    ...
    loaded = load_channel_rows(
        ...
        vibesdk_wrapper_log=args.vibesdk_wrapper_log,
    )
