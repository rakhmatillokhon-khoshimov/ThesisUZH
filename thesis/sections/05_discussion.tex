...
The completed scale up supports a narrower claim than saying that the same model
behaves differently because of access channel. The study compares a bare OpenAI API
...
interpretive move is to tier those cases. Ten are response core cases. Six are
...
the smaller refusal, factuality, and format set.
...
binary refusal or compliance labels. Binary labels are useful, but they can hide
...
positive findings in Categories B and F. It also narrows the thesis claim. The
observed deployment surface difference is not a generic social sensitivity
...
deep. They show why app audits need screenshot-backed evidence and
...
question. It asks what happens under the bare API condition. The consumer app is a
...
$p=0.0625$ for full refusal discordance, which is directional but not significant
...
neither should be overread. First, the retained cases are not uniform across the
...
That makes the central refusal and factuality finding more defensible than a pure
...
final case has an anchor other than safety framing: visible UI surface, factual
...
screenshot correction, the thesis would have reported a false format failure for
S038 and an inflated social over-refusal story. The
...
OpenAI API, because the plain OpenAI Responses API issues far more bare full
refusals than any other channel in this collection (five, versus at most one
elsewhere). This is a useful warning against
...
mechanisms or as an additional primary finding. The same pattern is also
consistent with the app serving a different or newer model than the API's
\texttt{gpt-4o-2024-08-06} snapshot. A bare \texttt{gpt-4o} call resembling other
vendors' APIs less than the app does cannot, on black-box evidence, separate a
wrapper effect from a model version effect. The agreement structure therefore
reinforces the deployment stack confound of Chapter~6 rather than resolving it,
which is why it is reported as context and not as a mechanism.
