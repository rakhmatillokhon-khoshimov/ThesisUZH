...
The completed scale-up supports a deployment-surface claim, not a same-model
causal claim. The study compares a bare OpenAI API condition with the official
productized ChatGPT app deployment and interprets only observable differences
...
The primary McNemar result does not reach $\alpha=.05$, so it should not be sold
as a confirmatory finding. Its role is to summarize the refusal direction and
keep the thesis honest about uncertainty. The argument therefore rests on the
case inventory, corrected evidence chain, and repetition results rather than on a
single p-value.
...
reinforces the general methodological point that a single access point should not
stand in for an unevaluated deployment surface.
...
The thesis does not show that one deployment surface is globally better or that a
particular hidden policy caused the differences. The correct interpretation is
narrower and stronger: under documented collection conditions, the bare API
condition and productized app deployment produced different observable behavior
on a frozen benchmark-grounded prompt bank. Those differences are concrete enough
that deployment surface should be recorded and audited, not treated as a neutral
