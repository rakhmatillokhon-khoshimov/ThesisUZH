...
\paragraph{Construct: the deployment stack.}
The comparison construct is the observable deployment stack. The API condition is
explicitly fixed to \texttt{gpt-4o}; the app condition is the official ChatGPT
consumer deployment observed during collection, where screenshots do not
reliably preserve a model selector or footer label. The app may include model
routing, hidden instructions, tools, retrieval, UI rendering, session state, and
account or location context. The study therefore compares a documented bare API
surface with the product surface a user actually meets. It does not attribute an
observed difference to one hidden component.
...
The app run was collected manually because the thesis prioritizes a traceable
consumer-surface record: one full screenshot, one transcript, and one reviewable
evidence row per prompt. This choice improves auditability but creates the
study's main scaling bottleneck. Browser automation with Playwright or Selenium,
DOM capture where available, and OCR-backed screenshot checks would be the
engineering path to hundreds of app prompts. The present design should therefore
be read as a high-care diagnostic collection, not as the scalable endpoint of the
method.
...
The study is underpowered for a confirmatory empirical NLP claim. With 60 paired
prompts, the primary endpoint depends on the discordant pairs, not on the full
denominator. The observed refusal table contains five API-only full refusals and
zero app-only full refusals; under the exact two-sided McNemar test this gives
$p=0.0625$. One additional one-directional discordant pair would cross the
conventional threshold ($2\cdot0.5^6=0.03125$), which shows how little margin the
test has at this scale. The thesis therefore uses the test as a disciplined
summary of a case-based audit rather than as the sole proof of a population-level
effect.
...
descriptors and claim-safe summaries.
