...
conservative. It is a fixed prompt black box comparison that reports only
observable differences and does not attribute them to any single hidden
component. The sections below set out the study design and comparison construct, the frozen
...
\begin{itemize}
  \item OpenAI Responses API using \texttt{gpt-4o}, with no custom system prompt
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{../figures/study_design_pipeline.pdf}
...
try to. Every comparative result should therefore be read as evidence that the
bare API differs from the productized deployment. That is precisely the contrast
an ordinary user is exposed to, but it is not evidence of any single hidden
...
used to test the feasibility of paired API and app collection, screenshot evidence,
...
\begin{longtable}{@{}L{0.055\linewidth}L{0.27\linewidth}rL{0.54\linewidth}@{}}
...
ambiguous context items (10 prompts rewritten into advice or recommendation form),
...
structured metadata. The scale up API collection completed all 60 rows. API
outputs are stored in private logs for full review and in shared or generated
summaries for thesis reporting.
...
\path{chatgpt.com}. The main 60 prompt app collection was conducted after the
OpenAI API scale up run in the same audit window. Each row requires a
screenshot, a transcribed output, and metadata about visible model labels or UI
surfaces where available. The archived
...
ChatGPT deployment observed in the app condition. This weakens model family
comparability relative to an ideal controlled design, and it
...
the log records \path{session_reset=not_visible_from_screenshot} rather
...
evidence. The visible output nevertheless contains Zurich or Switzerland specific
...
during the same audit window. Claude used the Anthropic Messages API with
...
For the OpenAI API channel, the first scale up run was the primary run. Two
additional OpenAI API runs were collected later using the same 60 prompt bank
and the same request recipe: \texttt{gpt-4o}, no custom system
...
Claude API repetitions were also collected as API only robustness context.
Gemini repetition stability is not reported because the repeated Gemini
...
the 17 final substantive divergence cases. The repetition checklist required a
fresh ChatGPT chat for each row, exact prompt pasting, no retry or
...
has a screenshot and nonempty output. Second, prompt echo cleaning removes
...
\begin{itemize}
  ...
  \item corrected or excluded: a row whose earlier label depended on transcription
  or coding error.
\end{itemize}
...
nonempty non full refusal outcomes are coded as 0. Thus \texttt{answered} and
...
\paragraph{One primary endpoint.}
...
comparison in this thesis is treated as exploratory. This includes the category,
source family, and risk level associations, the between category contrasts, the
lexical cross checks, the within channel versus between channel similarity
comparison, the refusal style marker counts, and the four channel agreement
summaries. Their p values are not used as independent confirmation of
the thesis claim.
...
overreading small category counts, not to imply that each category cell is
separately powered.
...
divergence prompts and a purposive set of rows not coded as divergent, yielding
27 prompts and 54 coding units. The purposive sample intentionally overweights
...
The second coder procedure was used after supervisor approval. The coder was outside
...
The boundary is that private high risk prompts and raw outputs remain local, and
live app behavior cannot be replayed exactly. The shared thesis uses redacted
descriptors and release safe summaries.
