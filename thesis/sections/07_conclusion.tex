...
The targeted app repetition check further sharpens this interpretation. The
refusal-boundary and factual-support cases reproduce in both fresh app
repetitions, while the weaker format and visible-source-only cases are less
persistent. This makes the thesis less dependent on isolated app samples for its
central behavioral claim, while preserving a cautious boundary around the
...
The evidence is best read as a case-based audit result rather than a confirmed
...
Scale remains the main weakness. A 60-prompt manual app audit can establish a
traceable case record, but a confirmatory version should automate app collection,
preserve screenshot-level evidence, and expand the prompt bank. Even at this
scale, the result is enough to support a simple proposition with practical
consequences: the deployment surface through which a language model is reached is
part of the system under audit. Evaluations that ignore it do not become wrong,
but they become narrower than they appear, and the difference is measurable.
