...
PROJECT = ROOT.parent
ANALYSIS = PROJECT / "pilot_outputs" / "20260608" / "analysis_scaleup_cleaned"
OUT = ROOT / "generated"
...
def write_prompt_bank(rows: list[dict[str, str]]) -> None:
    lines = [
        ...
        r"\caption{Redacted 60-prompt scale-up bank. Category B prompt text remains redacted in shared thesis artifacts; the private local bank contains executable collection text.}\\",
        r"\toprule",
...
def write_substantive(rows: list[dict[str, str]]) -> None:
    lines = [
        ...
        r"\caption{Final 17 substantive API/app divergence cases after prompt-echo cleaning and screenshot-backed corrections.}\\",
        r"\toprule",
...
def write_reviewed(rows: list[dict[str, str]]) -> None:
    lines = [
        ...
        r"\caption{Reviewed API/app coding summary for all 60 paired prompts. The table reports final claim-safe labels only, not raw high-risk prompt or output text.}\\",
        r"\toprule",
...
def write_cross(reviewed: list[dict[str, str]], api_rows: list[dict[str, str]]) -> None:
    ...
    lines = [
        ...
        r"\caption{Cross-channel refusal-status matrix. Claude and Gemini are API-only robustness baselines; only OpenAI API versus ChatGPT app is a channel comparison.}\\",
        r"\toprule",
...
def main() -> None:
    ...
    prompt_rows = read_csv(PROJECT / "prompt_bank_60.csv")
