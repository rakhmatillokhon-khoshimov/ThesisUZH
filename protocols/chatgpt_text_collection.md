...
**Purpose:** define the consumer-app collection standard. Screenshot-based
collection is acceptable if transcription quality is checked; direct text export
remains preferable when easy, but is not required for scale-up.

## Status

The screenshot run is usable as evidence that all prompts were submitted to the
ChatGPT website, including the high-risk refusal prompts, provided that
screenshot-to-text transcription is verified.
...
1. Open a new chat.
2. Paste exactly the prompt text from the restricted prompt bank.
3. Wait until the response is complete.
4. Copy the full model response text into `raw_output` if possible; otherwise transcribe from the screenshot.
...
7. Save a screenshot to `data/private/YYYYMMDD/chatgpt_app_clean_text/screenshots/Pxx.png`.
...
`data/private/YYYYMMDD/chatgpt_app_collection/chatgpt_app_log_private.csv`

Use the same fields as:

`data/private/20260527/chatgpt_app_manual_screenshots/chatgpt_app_log_private.csv`
